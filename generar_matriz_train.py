...
Script:   generar_matriz_train.py
Autor:    Cristian Acevedo

Descripcion:
    - Evalua el conjunto de entrenamiento (train) del dataset local.
    - Genera:
        * Matriz de confusion (PNG + CSV)
        * Reporte de clasificacion (precision, recall, f1-score)
        * Accuracy global sobre TRAIN
...
class Config:
    ...
    # Dataset local: dataset/tomato/train/...
    DATASET_ROOT = BASE_DIR / "dataset" / "tomato"
    SUBDIR = "train"

    # Salidas
    OUTPUT_PNG = BASE_DIR / "matriz_confusion_train.png"
    OUTPUT_CSV = BASE_DIR / "matriz_confusion_train.csv"
    LOG_PATH = BASE_DIR / "resumen_evaluacion_train.txt"

    # Nombres abreviados para el grafico
    TRADUCCIONES = {
...
def main():
    ...
    train_dir = Config.DATASET_ROOT / Config.SUBDIR
    ...
    print(f"\nLeyendo imagenes desde: {train_dir}")
    images = [p for p in train_dir.rglob("*") if p.suffix in exts]
    images.sort()
    ...
    for img_path in images:
        ...
        # Carpeta indica la clase real (Tomato___Early_blight, etc.)
        ...
        if true_id is None:
            # Si la carpeta no se encuentra en labels.json, se omite
    ...
    if total == 0:
        print("No se proceso ninguna imagen del conjunto train.")
        sys.exit()

    print(f"\nTotal de imagenes evaluadas (TRAIN): {total}")
    ...
    plt.ylabel("Clase real (train)")
    plt.xlabel("Prediccion del modelo")
    plt.title(f"Matriz de confusion - TRAIN (n={total})")
    ...
    # Reporte y accuracy global
    print("\n--- REPORTE DETALLADO (solo TRAIN) ---")
    ...
    acc = aciertos / total
    print(f"Accuracy global TRAIN: {acc:.3f} ({aciertos}/{total})")

    # Log en TXT
    try:
        with open(Config.LOG_PATH, "w", encoding="utf-8") as f:
            f.write("Resumen de evaluacion del modelo (TRAIN)\n")
